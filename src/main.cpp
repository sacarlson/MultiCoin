...
bool CommitEscrowTransaction(CWalletTx& wtxNew, uint256 hashInputTx, CReserveKey& reservekey)
{
    CRITICAL_BLOCK(cs_main)
    {
        printf("CommitTransaction:\n%s", wtxNew.ToString().c_str());
...
//
pair<string,string> SendMoneyFromEscrow(string strAddress, uint256 hashInputTx, string strPartialTx, CWalletTx& wtxNew, bool fAskFee)
{
    CReserveKey reservekey;
    ...
    if (VerifyScript(scriptSigNew, scriptPubKey, wtxNew, 0, 0))
    {
        ...
        if (!CommitEscrowTransaction(wtxNew, hashInputTx, reservekey))
...
// requires cs_main lock
string SendMoneyToEscrow(string strAddresses, int64 nValue, CWalletTx& wtxNew, bool fAskFee)
{
    // Check amount
    if (nValue <= 0)
        return _("Invalid amount");
